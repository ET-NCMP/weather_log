...
def get_climate_files(month):
    ...
    ## MSLP
    f1 = Dataset("mslp_climate_era5_1981_2010.nc",'r')
    mslp_climate = f1.variables['mslp_climate'][moidx]
    f1.close()

    ## Z500 and thickness
    f2 = Dataset("z500_thickness_climate_era5_1981_2010.nc",'r')
...
def get_current_data(year, month):
	...
	## MSLP
	f1 = Dataset("current_month/era5_monthly_weatherlog_mslp_"+str(year)+"_"+str(month)+".nc",'r')
	...
	# Z500 and thickness
	f2 = Dataset("current_month/era5_monthly_weatherlog_plev_"+str(year)+"_"+str(month)+".nc",'r')
...
###=============================================================================
## user specify values (update in future to pass via command line)
year_plot=2021
month_plot=5
DPI=600 

## No need to do anything beyond this line
##==============================================================================
mslp_climate,z500_climate,thickness_climate = get_climate_files(month_plot)
mslp_month, z250_month,z500_month, thickness_month, wind_month,lons, lats = get_current_data(year_plot,month_plot)
...
## linestyle for these
ls='-'; lw=0.4; c='gray'; fontsize_title=6.5
...
## Thickness
cf2 = axs[1].contourf(lons, lats, thickness_anom/10,cmap=cmap_thick, transform=trans,levels=thickness_clevs,extend='both')
z_cr = axs[1].contour(lons,lats,thickness_month/10,colors='k',linewidths=0.25,levels=thickness_actual_levs,transform=trans)
...
msl_cr = axs[2].contour(lons,lats,mslp_month,colors='k',linewidths=0.25,levels=msl_actual_levs,transform=trans)
...
plt.savefig("weather_log_front_page_"+str(year_plot)+"_"+str(month_plot)+".png",dpi=DPI,bbox_inches='tight')
plt.close()
